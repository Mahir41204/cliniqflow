...
// Brevo/OTP email implementation disabled. Kept commented so it can be restored later.
...
// Manual email/password auth disabled. Google OAuth remains enabled.
...
async function findOrCreateGoogleUser(profile: {
  ...
  if (!existing) {
    // Google-only signup: create verified users directly without Brevo/OTP.
...
router.get("/auth/google/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Brevo/OTP verification for new Google users disabled. Kept for future restore.
    // if (!userRow) {
...
// Manual email/password registration disabled. Kept commented so it can be restored later.
...
// Manual email/password login disabled. Kept commented so it can be restored later.
...
// Manual credentials validation disabled with email/password auth.
...
// Brevo/OTP email endpoints disabled. Kept commented so they can be restored later.
